...
from app.db.schemas.traffic_lights import TrafficLight  # Импортируем Pydantic модель
from app.db.models.traffic_lights import TrafficLight as TrafficLightModel
...
async def change_traffic_light_state(light_id: str, new_state: str, db: AsyncSession = Depends(get_db)):
    ...
    return TrafficLight.from_orm(updated_light)  # Используем Pydantic метод from_orm
